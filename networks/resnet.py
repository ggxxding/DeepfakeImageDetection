...
from networks.patch_generator import smash_n_reconstruct
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        # torchvision.utils.save_image(x[0],'/home/ubuntu/dmj/DeepfakeImageDetection/origin.png')
...
class ResNetFull(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1, zero_init_residual=False):
        ...
        assert -1 < self.unfoldIndex and self.unfoldIndex < self.unfoldSize*self.unfoldSize
        self.inplanes = 64
        self.conv00 = nn.Conv2d(3, 32, kernel_size=3, padding = 1,bias=False)
        self.conv01 = nn.Conv2d(3, 32, kernel_size=3, padding = 1,bias=False)
    ...
    def forward(self, x):
        # print(x.shape)
        # x = smash_n_reconstruct(x)
        # torchvision.utils.save_image(x[0][:,0:32,0:32],'temp.jpg')
        # quit()
        # rich = x[:,0]

        rich = x[:,:,:256,:]
        poor = x[:,:,256:,:]
        NPR1 = (rich - self.interpolate(rich, 0.5))*2.0/3.0
        NPR2 = (poor - self.interpolate(poor, 0.5))*2.0/3.0
        # torchvision.utils.save_image(rich,'tempr.jpg')
        # torchvision.utils.save_image(poor,'tempp.jpg')
        NPR1 = self.conv00(NPR1)
        NPR2 = self.conv01(NPR2)
        NPR1 = self.bn00(NPR1)
        NPR2 = self.bn01(NPR2)
        NPR1 = self.hardtanh(NPR1)
        NPR2 = self.hardtanh(NPR2)
        x = NPR1-NPR2
...
def resnet50full(pretrained=False, **kwargs):
    model = ResNetFull(Bottleneck, [3, 4, 6, 3], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet50']))
    return model
