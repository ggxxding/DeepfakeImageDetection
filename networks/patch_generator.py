...
def img_to_patches(input_path) -> tuple:
    """
...
def get_complete_image(patches:list, coloured=True):
    ...
    p_len = len(patches)
    while len(patches)<49:
        patches.append(patches[random.randint(0, p_len-1)])
    
    if(coloured==True):
        grid = np.asarray(patches).reshape((7,7,32,32,3))
    else:
        grid = np.asarray(patches).reshape((7,7,32,32))


    # joins columns to only leave rows
    rows = [np.concatenate(grid[i,:], axis=1) for i in range(7)]
...
def smash_n_reconstruct(input_path, coloured=True):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        ...
        rich_texture = norm(pil_to_tensor(rich_texture_future.result()))
        poor_texture = norm(pil_to_tensor(poor_texture_future.result()))

    return rich_texture, poor_texture

if __name__=="__main__":
    for i in range(10):
        r,p = smash_n_reconstruct(input_path="./31temporigin.jpg")
        print(r.shape,p.shape)
        cv2.imwrite('rich1.jpg',r[:,:,::-1])
        cv2.imwrite('poor1.jpg',p[:,:,::-1])
        print(i)